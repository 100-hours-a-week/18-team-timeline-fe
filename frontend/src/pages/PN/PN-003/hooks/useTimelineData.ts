  ...
  const fetchNewsData = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        try {
          // 첫 번째 시도: 기본 URL 형식
          response = await axios.get<ApiResponse>(`${API_BASE_URL}/news/${numericId}`, axiosConfig)
          console.log('API 호출 성공')
        } catch (err1) {
          console.log('첫 번째 API 호출 실패, 대체 URL 시도:', err1)
          try {
            // 두 번째 시도: API 버전이 포함된 URL 형식
            response = await axios.get<ApiResponse>(`${API_BASE_URL}/api/v1/news/${numericId}`, axiosConfig)
          } catch (err2) {
